...
class KeysetHelper {
  ...
  static Future<KeysetInfo?> tryGetMintKeysetInfo(
    IMint mint,
    String unit, [
    String? keysetId,
  ]) async {
    ...
    // from local
    final keysets = await KeysetStore.getKeyset(
        mintURL: mint.mintURL, id: keysetId, unit: unit, active: true);
    ...
    if (keysetInfo == null || keysetInfo.keyset.isEmpty) {
      ...
      keysetInfo = findBetterKeyset(
          newKeysets.where((element) => element.unit == unit).toList());
    }
  ...
  static Future<List<KeysetInfo>> fetchKeysetFromRemote(
    IMint mint, [
    String? keysetId,
  ]) async {
    ...
    // Fetch keysets keys
    final keysResponse =
        await mint.requestKeysAction(mintURL: mint.mintURL, keysetId: keysetId);
    final keys =
        keysResponse.isSuccess ? keysResponse.data : <MintKeysPayload>[];
    ...
    // Update local data
    final localKeysets =
        (await KeysetStore.getKeyset(mintURL: mint.mintURL)).map((info) {
      cache[info.id] = info;
      return info;
    }).toList();
    if (mint.maxNutsVersion >= 1) {
      final stateResponse =
          await Nut2.requestKeysetsState(mintURL: mint.mintURL);
      List<KeysetInfo> list =
          stateResponse.isSuccess ? stateResponse.data : <KeysetInfo>[];
  ...
  static Future<MintKeys?> keysetFetcher(
    String mintURL,
    String unit,
    String keysetId,
  ) async {
