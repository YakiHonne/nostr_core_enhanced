...
class ProofRequest {
  ...
  ProofRequest.amount(this.amount) : proofs = null;
...
class ProofHelper {
  static Future<List<Proof>> getProofs(
    ...
    final usableProofs = await ProofStore.getProofs(
      mintURL: mintURL,
      ids: keysets.map((e) => e.id).toList(),
    );
  ...
  static Future<CashuResponse<List<Proof>>> swapProofs({
    ...
    if (blindedMessages.isEmpty) {
      return CashuResponse.fromErrorMsg('blindedMessages is empty.');
    }
  ...
  /// Verifies proofs against the mint and returns partitioned lists of live and burned proofs.
  static Future<(List<Proof> live, List<Proof> burned)> verifyProofStates({
    required IMint mint,
    required List<Proof> proofs,
  }) async {
    final live = <Proof>[];
    ...
    if (!response.isSuccess) {
      throw Exception('Failed to check proofs: ${response.errorMsg}');
    }

    if (response.data.length != proofs.length) {
      throw Exception(
          'State check length mismatch: ${response.data.length} vs ${proofs.length}');
    }
  ...
  /// No-op: State changes equate to new Token Events.
  static Future<void> saveProofStates(List<Proof> proofs) async {}
  ...
  //   var totalProofs =
  //       proofRequest.proofs ?? await getProofs(mint.mintURL, true);
  //   // Prioritize stored live proofs over pending without extra network call
  //   totalProofs.sort((a, b) {
  //     int priority(Proof p) => p.state == TokenState.inFlight ? 1 : 0;
  //     final diff = priority(a) - priority(b);
  //     if (diff != 0) return diff;
  //     return b.amountNum.compareTo(
  //       a.amountNum,
  //     ); // keep larger amounts first (existing behavior)
  //   });
  ...
  //   result =
  //       _findOneSubsetWithSum(totalProofs, amount, keysetInfo.inputFeePPK) ??
  //           [];
  //   if (result.isNotEmpty) {
  //     return ProofResponse.success(
  //       targetAmount: proofRequest.amount,
  ...
  //   if (response.data.totalAmount != request.amount) {
  //     return CashuResponse.fromErrorMsg('Invalid amount proofs.');
  //   }
  //   return response;
  // }
  ...
  //   List<BlindedMessage> blindedMessagesSet =
  //       blindedMessages.expand((e) => e).toList();
  //   List<String> secretsSet = secrets.expand((e) => e).toList();
  //   List<BigInt> rsSet = rs.expand((e) => e).toList();
  //   List<UnblindingOption> unblindingOptionSet =
  //       unblindingOptions.expand((e) => e).toList();
  //   if (blindedMessages.isEmpty) {
  //     return CashuResponse.fromErrorMsg('blindedMessages is empty.');
  //   }
  ...
  // static List<Proof>? _findOneSubsetWithSum(
  //     List<Proof> proofs, int target, int inputFeePPK) {
  //   int adjustedTarget(int subsetLength) {
  //     return target +
  //         ((subsetLength * inputFeePPK + 999) ~/
  //             1000); // ceil(length * inputFee / 1000)
  //   }
  ...
  //             (dp[newSum]!.length > currentSubset.length)) {
  ...
  //         // Early return if the current subset matches the required sum
  //         if (currentSubset.fold<int>(0, (sum, value) => sum + value) ==
  //             requiredSum) {
  //           return currentSubset;
  //         }
  ...
  //       );
  //     case ConditionType.htlc:
  //       await WitnessHelper.addHTLCWitnessToProof(
  //         proof: proof,
  ...
  //   final proofs = await ProofStore.getProofs(mintURL: '');
