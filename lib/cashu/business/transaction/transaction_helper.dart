...
class TransactionHelper {
  ...
  static Future<CashuResponse<List<Proof>>> requestTokensFromMint({
    ...
    if (!response.isSuccess) {
      if (tryUpdateKeysetsIfNeeded &&
              response.errorMsg.contains('keyset id unknown') ||
          response.errorMsg.contains('keyset id inactive')) {
  ...
  static Future<CashuResponse<(MeltResponse, List<Proof>)>> payingTheQuote({
    required IMint mint,
    String paymentId = '',
    ...
    final (blindedMessages, secrets, rs, _) = DhkeHelper.createBlankOutputs(
      keysetId: keysetInfo.id,
      amount: proofs.totalAmount - amount - fee,
    );
    ...
    if (!meltResponse.isSuccess) return meltResponse.cast();
    ...
    return CashuResponse.fromSuccessData((meltResponse.data, newProofs));
  }
}
