...
class InvoiceHandler {
  ...
  Future<void> initialize() async {
    ...
    Future.delayed(
        const Duration(seconds: 10), () => invoiceChecker?.initComplete());
  }
    ...
    try {
      ...
      if (paid) {
        ...
        if (response.isSuccess) {
          ...
          return true;
        } else if (response.code == ResponseCode.invoiceNotPaidError &&
            invoice.isExpired) {
          deleteInvoice(invoice);
        }
      }
  ...
  bool _invoiceExists(Receipt invoice) {
    return _invoices
        .any((element) => element.redemptionKey == invoice.redemptionKey);
  }
}
