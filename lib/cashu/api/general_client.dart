...
// import 'package:nostr_core_enhanced/cashu/models/cashu_token_info.dart';
...
// import 'package:nostr_core_enhanced/cashu/utils/third_part_extension.dart';
  ...
  static Bolt11PaymentRequest? _tryConstructRequestFromPr(String pr) {
    ...
    try {
      ...
      for (var tag in req.tags) {
        logger.i(() => '[Cashu - invoice decode]${tag.type}: ${tag.data}');
      }
      return req;
    } catch (e, s) {
      logger.e(() => '[Cashu - invoice decode] error: $e, $s');
      return null;
    }
  }
}
...
//     logger.e(() => '[I][Cashu - sendEcash] Create Ecash: $encodedToken');
...
//       return CashuResponse.fromErrorMsg('Token entries is empty.');
//     }
//     if (token.unit.isNotEmpty && token.unit != 'sat') {
//       return CashuResponse.fromErrorMsg('Unsupported unit');
//     }
...
//             // Keyset info
//             final keysetInfo =
//                 await KeysetHelper.tryGetMintKeysetInfo(mint, token.unit);
...
//                   if (result == false) {
//                     logger.i(() =>
//                         '[DLEQ - verify] verification failed, proof: $proof, pk: $pk');
//                   } else {
//                     newProofs.add(proof);
//                   }
//                 }
...
//               'No funds available proofs for redemption.');
//         }
//       } catch (e, stack) {
...
//             .where((history) => num.parse(history.amount) < 0)
//             .forEach((history) {
...
//     }
//     final hash =
//         req.tags.where((e) => e.type == 'payment_hash').firstOrNull?.data;
...
//       final response = await mint.tokenCheckAction(
//           mintURL: mint.mintURL, proofs: originProofs);
//       if (!response.isSuccess || response.data.length != originProofs.length) {
//         return null;
//       }
...
//       newEntries.add(TokenEntry(proofs: spendableProofs, mint: mint.mintURL));
//     }
...
//     final proofs =
//         token.entries.fold(<Proof>[], (pre, e) => pre..addAll(e.proofs));
