...
class DhkeHelper {
  static BlindedMessageData _createBlindedMessages({
    ...
    for (var i = 0; i < amounts.length; i++) {
      ...
      final secret = secretCreator?.call(amount) ??
          CryptoUtils.randomPrivateKey().asBase64String();
  ...
  static BlindedMessageData createDeterministicBlindedMessages({
    ...
    for (var i = 0; i < batchSize; i++) {
      final currentCounter = counter + i;
      final secretBytes = Nut13.deriveSecret(seed, keysetId, currentCounter);
      final rBytes = Nut13.deriveBlindingFactor(seed, keysetId, currentCounter);

      final secret = secretBytes.asHex();
      final r = rBytes.asBigInt();

      final (B_, _) = Dhke.blindMessage(secret, r);
      if (B_.isEmpty) continue;
  ...
  static Uint8List get domainSeparator =>
      'Secp256k1_HashToCurve_Cashu_'.asBytes();

  static String hashToCurve(String message) {
    Uint8List messageBytes = message.asBytes();
    Uint8List msgToHash = Uint8List.fromList(
      sha256.convert([...domainSeparator, ...messageBytes]).bytes,
    );
