...
class HTLCSecret with Nut10Secret {
  ...
  static HTLCSecret? fromOptions({
    ...
    final lockTimeTimestamp = lockTime != null
        ? (lockTime.millisecondsSinceEpoch ~/ 1000).toString()
        : null;

    final htlcSecretTags = [
      if (publicKeys.isNotEmpty)
        HTLCSecretTagKey.pubkeys.appendValues(publicKeys),
      if (refundPubKeys != null && refundPubKeys.isNotEmpty)
        HTLCSecretTagKey.refund.appendValues(refundPubKeys),
      if (lockTimeTimestamp != null)
        HTLCSecretTagKey.lockTime.appendValues([lockTimeTimestamp]),
    ];
...
class HTLC {
  static (String preimage, String hashString) createHashData(
      [String? preimage]) {
