...
class Nut13 {
  ...
  static Uint8List deriveBlindingFactor(
    ...
    return derive(
        seed, keysetId, counter, DerivationType.blinding_factor.value);
  }

  static Uint8List derive(
      Uint8List seed, String keysetId, int counter, int secretOrBlinding) {
  ...
  static int getKeysetIdInt(String keysetId) {
    BigInt mod = BigInt.from(pow(2, 31) - 1);
    BigInt keysetIdBig;

    // v1 keyset ID (hex)
    if (keysetId.length == 16 && RegExp(r'^[a-fA-F0-9]+$').hasMatch(keysetId)) {
      keysetIdBig = BigInt.parse(keysetId, radix: 16);
    } else {
      // v0 keyset ID (base64)
      try {
        final bytes = base64.decode(keysetId);
        keysetIdBig = BigInt.parse(toHexString(bytes), radix: 16);
      } catch (e) {
        // fallback or error
        keysetIdBig = BigInt.zero;
      }
    }

    return (keysetIdBig % mod).toInt();
  }
}
