...
class Token {
  ...
  BigInt get sumProofsValue =>
      entries.fold(BigInt.zero, (pre, entry) => pre + entry.sumProofsValue);
...
class TokenEntry {
  ...
  BigInt get sumProofsValue =>
      proofs.fold(BigInt.zero, (pre, proof) => pre + proof.amount.asBigInt());
  ...
  factory TokenEntry.fromJson(Map json) {
    ...
    if (proofsJson is List) {
      proofs = proofsJson
          .map((e) {
            if (e is Map<String, Object?>) {
              return Proof.fromServerJson(e, fromDb: true);
            }
            return null;
          })
          .where((element) => element != null)
  ...
  List toV4Json() {
    ...
    return allProofs.entries
        .map((entry) => {
              "i": (entry.key.length % 2 == 0 && entry.key.isHexString())
                  ? entry.key.hexToBytes()
                  : entry.key.base64AsBytes(),
              "p": entry.value,
            })
        .toList();
  }
